...
from   xml.dom.ext     import PrettyPrint #@UnresolvedImport
...
class KML(object):
  ...
  def createIcon(self, iconUrl=None, x=None, y=None, w=None, h=None, dim=None):
    ...
    for d in dimensions:
      if d[1] != None:
        ...
        elt.appendChild(self.xml.createTextNode(str(d[1])))
  ...
  def createIconStyle(self, scale=None, icon=None, color=None):
    ...
    if scale != None:
      ...
      elt.appendChild(self.xml.createTextNode(str(scale)))
  ...
  def createLookAt(self, lat, lon,  range=None, tilt=None, heading=None):
    ...
    if range != None:
      ...
      elt.appendChild(self.xml.createTextNode(str(range)))
      pt.appendChild(elt)
    if tilt != None:
      elt = self.xml.createElement("tilt")
      elt.appendChild(self.xml.createTextNode(str(tilt)))
      pt.appendChild(elt)
    if heading != None:
      elt = self.xml.createElement("heading")
      elt.appendChild(self.xml.createTextNode(str(heading)))
      pt.appendChild(elt)
    return pt
